      ...
      m_ModListWriter(std::bind(&Profile::doWriteModlist, this))
{
  ...
  m_Settings = new QSettings(directory.absoluteFilePath(u"settings.ini"_s),
                             QSettings::IniFormat);
  findProfileSettings();
...
void Profile::doWriteModlist()
{
  ...
  try {
    ...
    file->write(
        (QStringLiteral("# This file was automatically generated by Mod Organizer.") %
         newline)
            .toUtf8());
...
{
  return setting("", u"LocalSaves"_s, Settings::instance().profileLocalSaves())
      .toBool();
}
...
Profile::determineForcedLibraries(const QString& executable) const
{
  ...
  for (auto forcedLoad : forcedLoads) {
    ...
    for (const auto& rawSetting : rawSettings) {
      if ((rawSetting.value(u"process"_s)
               .toString()
               .compare(forcedLoad.process(), Qt::CaseInsensitive) == 0) &&
          (rawSetting.value(u"library"_s)
               .toString()
               .compare(forcedLoad.library(), Qt::CaseInsensitive) == 0)) {
        results.append(
  ...
  for (const auto& rawSetting : rawSettings) {
    ...
    if (add) {
      results.append(
          ExecutableForcedLoadSetting(rawSetting.value(u"process"_s).toString(),
