                                  ...
                                  const MOShared::DirectoryEntry& directoryEntry)
{
  ...
  for (auto& modIndex : modIndices) {
    ...
    if (!selectedMod.isNull() && profile->modEnabled(modIndex)) {
      ...
      QStringList plugins =
          dir.entryList(QStringList() << u"*.esp"_s << u"*.esm"_s << u"*.esl"_s);
...
QVariant PluginList::tooltipData(const QModelIndex& modelIndex) const
{
  ...
  if (esp.description.size() > 0) {
    toolTip += u"<br><b>"_s % tr("Description") % u"</b>: "_s %
  ...
  if (!enabledMasters.empty()) {
    toolTip += u"<br><b>"_s % tr("Enabled Masters") % u"</b>: "_s %
               TruncateString(SetJoin(enabledMasters, u", "_s));
  }
  ...
  if (esp.hasNoRecords) {
    toolTip +=
        u"<br><br>"_s % tr("This is a dummy plugin. It contains no records and is "
  ...
  if (esp.forceDisabled) {
    ...
    if (feature && esp.hasLightExtension && feature->lightPluginsAreSupported()) {
      toolTip +=
          u"<br><br>"_s % tr("Light plugins (ESL) are not supported by this game.");
    } else {
