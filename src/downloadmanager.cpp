...
DownloadManager::DownloadManager(NexusInterface* nexusInterface, QObject* parent)
    : m_NexusInterface(nexusInterface), m_ParentWidget(nullptr), m_DirWatcher(this),
      m_ShowHidden(false)
{
...
void DownloadManager::refreshList()
{
  ...
  try {
    ...
    env::forEachEntry(
        ...
        [](void* data, QStringView f, QDateTime, uint64_t size) {
          auto& cx = *static_cast<Context*>(data);

          QString lc = MOShared::ToLowerCopy(f);
